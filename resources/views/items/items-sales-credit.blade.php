{{--
    resources/views/sales-credit/index.blade.php
    Complete working credit sales page with barcode and name search
    Fully responsive for all devices (mobile, tablet, desktop)
    FIXED: Scanner camera stability issue
    FIXED: Barcode input accepts only numbers (English/Persian/Arabic/Kurdish)
...
<title>{{ config('app.name', 'Laravel') }} - فرۆشتنی قەرز</title>
...
<div class="sales-container">
    <div class="form-header-custom text-white">
        ...
        <div style="display: flex; gap: 12px; justify-content: center; margin-top: 16px; flex-wrap: wrap;">
            <a href="{{ route('sales.index') }}" class="btn-credit" style="width: auto; padding: 8px 16px;">
        ...
        <a href="{{ route('debtors.save-products') }}" class="btn-return mt-3" style="width: auto; padding: 8px 16px;">
...
function searchByExactBarcode(barcode) {
    ...
    $.ajax({
        url: '/products/search-by-barcode',
        type: 'POST',
        data: { search_term: cleanBarcode, _token: '{{ csrf_token() }}' },
        success: function(res) {
